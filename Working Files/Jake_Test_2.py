...
def remove_entry(state, var_name, action, payload):
    index = payload["index"]
    state.data = state.data.pop(list(state.data.kets())[index])
...
<center>
<|{data}|table|columns={columns[0]}|show_all|width=1000px|on_delete=remove_entry|>
<|{columns}|toggle|>
</center>
